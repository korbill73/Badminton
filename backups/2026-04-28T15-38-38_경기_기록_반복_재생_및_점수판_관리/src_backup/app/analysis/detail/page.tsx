...
import { 
    ...
    BarChart3, Video, RefreshCcw, Maximize2, Layers, ChevronLeft, ChevronRight, PlayCircle, StopCircle, Loader2, RotateCw
} from 'lucide-react';
...
const AnalysisMobileView = ({ 
    ...
    useEffect(() => {
        const initPlayer = () => {
            const videoId = match.youtube_video_id;
            if (!videoId) return;
                                ...
                                <div key={l.id} className={cn(
                                    ...
                                    <div className="flex items-center justify-between">
                                        ...
                                        <div className="flex items-center gap-2 shrink-0 bg-black/50 p-2 rounded-2xl border border-white/10 shadow-lg">
                                            ...
                                            <button onClick={() => deletePoint(l.id, l.set_number)} className="ml-1 p-2 text-white/20 hover:text-rose-500 transition-all opacity-0 group-hover/row:opacity-100 hover:rotate-12 active:scale-75"><Trash2 className="w-4 h-4" /></button>
                                        </div>
                                    </div>
                                </div>
                            );
