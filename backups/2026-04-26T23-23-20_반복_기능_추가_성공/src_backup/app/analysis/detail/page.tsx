...
function CockpitAnalysisContent() {
    ...
    return (
        <div className="flex flex-col h-screen bg-[#080d1a] text-slate-100 font-sans overflow-hidden">
            ...
            {/* STICKY HEADER */}
            <div className="h-auto md:h-16 bg-[#0b1221] border-b border-white/5 flex flex-col md:flex-row items-center justify-between px-4 md:px-8 py-2 md:py-0 shrink-0 z-50 gap-3 md:gap-0">
                <div className="flex items-center justify-between w-full md:w-auto">
                    <div className="flex items-center gap-2 md:gap-6">
                        ...
                        <div className="flex flex-col group cursor-pointer" onClick={() => setIsMatchModalOpen(true)}>
                            <h1 className="text-xs md:text-sm font-black text-white group-hover:text-blue-400 transition-colors flex items-center gap-2 truncate max-w-[120px] md:max-w-none">
                                {match?.match_name} <Edit2 className="hidden md:block w-3 h-3 opacity-0 group-hover:opacity-100 transition-all text-blue-400" />
                            </h1>
                            <div className="flex items-center gap-2 mt-0.5">
                                <span className="text-[9px] md:text-[10px] text-blue-500 font-black truncate max-w-[150px] md:max-w-none">
                                    vs {match?.opponent_1?.name}{match?.opponent_1?.school ? ` (${match.opponent_1.school})` : ''}
                                </span>
                            </div>
                        </div>
                    </div>
                    {/* Scores for Mobile - Row 1 Right */}
                    <div className="md:hidden flex items-center gap-3 font-black bg-white/5 px-4 py-1.5 rounded-xl border border-white/10">
                        <span className="text-xl text-blue-400 tabular-nums">{sMe}</span>
                        <span className="text-lg text-white/20 italic">:</span>
                        <span className="text-xl text-rose-500 tabular-nums">{sOpp}</span>
                    </div>
                </div>

                {/* View Toggles & Mobile Sets (Sub Row) */}
                <div className="flex flex-col md:flex-row items-center gap-3 w-full md:w-auto">
                    <div className="flex items-center p-1 md:p-1.5 bg-[#080d1a] md:bg-[#0b1221] rounded-xl md:rounded-2xl gap-0.5 md:gap-1 border border-white/10 shadow-2xl relative shrink-0 w-full md:w-auto overflow-hidden">
                        <button 
                            onClick={() => { setActiveView('video'); if (currentSet === 0) setCurrentSet(1); }} 
                            className={cn("flex-1 md:w-40 py-2 md:py-3 rounded-lg md:rounded-xl text-[10px] md:text-[12px] font-black transition-all uppercase tracking-widest flex justify-center items-center gap-2", activeView === 'video' ? "bg-blue-600 text-white shadow-[0_0_20px_rgba(37,99,235,0.4)] border border-blue-400/50" : "text-slate-400 hover:text-white hover:bg-white/5")}
                        >
                            영상 기록
                        </button>
                        <button 
                            onClick={() => setActiveView('report')} 
                            className={cn("flex-1 md:w-40 py-2 md:py-3 rounded-lg md:rounded-xl text-[10px] md:text-[12px] font-black transition-all uppercase tracking-widest flex justify-center items-center gap-2", activeView === 'report' ? "bg-rose-600 text-white shadow-[0_0_20px_rgba(225,29,72,0.4)] border border-rose-400/50" : "text-slate-400 hover:text-white hover:bg-white/5")}
                        >
                            <BarChart3 className="w-3.5 h-3.5 md:w-4 md:h-4" /> 통계 리포트
                        </button>
                    </div>

                    {/* Set Selection - Mobile New Line Policy */}
                    <div className="flex md:hidden items-center justify-center gap-1.5 bg-sky-500/5 p-1 rounded-xl border border-sky-500/10 w-full">
                        {[1, 2, 3].map(s => (
                            <button key={s} onClick={() => setCurrentSet(s)} className={cn("flex-1 py-2 rounded-lg text-[11px] font-black transition-all uppercase tracking-tight", currentSet === s ? "bg-cyan-400 text-black shadow-lg" : "text-slate-500 hover:text-white")}>{s}세트</button>
                        ))}
                    </div>
                </div>
                ...
                <div className="hidden md:flex items-center gap-8">
                    <div className="flex gap-2">
                        {activeView === 'report' && (
                            <button 
                                ...
                                className="px-6 py-2 bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-400 hover:to-orange-400 text-white rounded-xl font-black text-[12px] flex items-center gap-2 shadow-[0_0_20px_rgba(245,158,11,0.3)] transition-all disabled:opacity-50"
                            >
                    ...
                    <div className="flex items-center gap-4 font-black">
                        ...
                        <span className="text-2xl text-white/20 italic">:</span>
            ...
            {activeView === 'video' ? (
                <div className="flex-1 flex flex-col md:flex-row overflow-hidden p-1 md:p-2 gap-1 md:gap-2 animate-in fade-in duration-500">
                    
                    {/* COLUMN 1: WIN SIDEBAR (Left) - HIDDEN ON MOBILE */}
                    <div className="hidden md:flex w-[300px] bg-[#0b1221] border border-white/5 rounded-[2.5rem] p-6 overflow-y-auto custom-scrollbar flex flex-col gap-6 shadow-2xl shrink-0">
                    ...
                    {/* COLUMN 2: CENTER BLOCK (VIDEO + LOSS DASH) */}
                    <div className="flex flex-col md:flex-1 overflow-hidden gap-0 w-full shrink-0 md:shrink">
                        {/* VIDEO CONTAINER */}
                        <div className="w-full aspect-video bg-black rounded-xl md:rounded-t-[2.5rem] overflow-hidden border border-white/5 flex items-center justify-center p-0 shrink-0">
                            <div className="w-full h-full"><YoutubePlayer videoId={match?.youtube_video_id || ''} onPlayerReady={(p) => { playerRef.current = p; }} /></div>
                        </div>

                        {/* LOSS DASHBOARD - HIDDEN ON MOBILE */}
                        <div className="hidden md:flex flex-1 bg-[#0b1221] border-l border-r border-b border-white/10 p-6 gap-4 overflow-y-auto custom-scrollbar shadow-2xl rounded-b-[2.5rem]">
                            {lossCats.map((cat) => (
                    ...
                    {/* COLUMN 3: RALLY TIMELINE (Bottom on Mobile, Right on PC) */}
                    <div className="flex-1 md:w-[440px] md:flex-none bg-[#0b1221] rounded-2xl md:rounded-[2.5rem] border border-white/10 flex flex-col overflow-hidden shadow-2xl md:h-[750px] shrink-0 md:shrink">
                        <div className="px-4 md:px-6 py-3 md:py-5 border-b border-white/10 bg-slate-950/40 flex items-center justify-between">
                            <div className="flex items-center gap-3">
                                <Activity className="w-3.5 h-3.5 md:w-4 md:h-4 text-white/30"/>
                                <span className="text-[10px] md:text-[11px] font-black text-white/40 uppercase tracking-widest leading-none">RALLY TIMELINE (TOUCH TO SEEK)</span>
                            </div>
                            <div className="md:hidden text-[10px] font-bold text-blue-400">READ-ONLY VIEW</div>
                        </div>
                        <div className="flex-1 overflow-y-auto custom-scrollbar p-1.5 md:p-2 space-y-1 md:space-y-1.5 bg-black/40" ref={timelineRef}>
                            {cSetLogs.map((l) => {
                                ...
                                return (
                                    <div 
                                        key={l.id} 
                                        id={`log-${l.id}`} 
                                        onClick={() => playerRef.current?.seekTo(l.video_timestamp, true)} 
                                        className={cn(
                                            "group relative flex items-center px-4 md:px-6 py-2.5 md:py-4 rounded-xl md:rounded-2xl border transition-all duration-500 cursor-pointer overflow-hidden", 
                                            isActive 
                                                ? "bg-blue-600/20 border-blue-400 shadow-[0_0_30px_rgba(59,130,246,0.3)] scale-[1.02] z-10 backdrop-blur-md" 
                                                : "bg-slate-900/40 border-white/5 hover:border-white/10 hover:bg-slate-900/60"
                                        )}
                                    >
                                        {/* ACTIVE INDICATOR BAR */}
                                        <div className={cn(
                                            "absolute left-0 top-0 bottom-0 w-1 md:w-1.5 transition-all duration-500",
                                            isActive ? "bg-cyan-400 opacity-100 shadow-[0_0_15px_rgba(34,211,238,0.8)] animate-pulse" : "bg-white/0 opacity-0"
                                        )} />

                                        <div className={cn(
                                            "w-12 md:w-14 font-black text-sm md:text-lg tabular-nums transition-colors duration-500", 
                                            isActive ? "text-white drop-shadow-[0_0_8px_rgba(255,255,255,0.5)]" : (l.is_my_point ? "text-blue-400" : "text-rose-500")
                                        )}>
                                            {l.current_score}
                                        </div>
                                        ...
                                        <div className="flex-1 px-2 md:px-4">
                                            {editingLogId === l.id ? (
                                                <div className="flex items-center justify-between gap-1 bg-yellow-500/20 text-yellow-400 px-3 py-1.5 rounded-full text-[10px] md:text-[11px] font-black border border-yellow-500/30">
                                                    <span>수정 중...</span>
                                                ...
                                                <div className={cn(
                                                    "px-3 md:px-4 py-1 md:py-1.5 rounded-full uppercase transition-all duration-500 flex items-center justify-center gap-1 md:gap-1.5 border", 
                                                    isActive 
                                                        ? "bg-cyan-500/20 border-cyan-400/50 text-white" 
                                                        : (l.is_my_point ? "bg-blue-500/10 border-transparent" : "bg-rose-500/10 border-transparent")
                                                )}>
                                                    {pCat && (
                                                        <>
                                                            <span className={cn(
                                                                "text-[11px] md:text-[13px] font-black transition-colors duration-500", 
                                                                isActive ? "text-cyan-300" : (l.is_my_point ? "text-blue-400" : "text-rose-400")
                                                            )}>{pCat}</span>
                                                            <span className="text-xs md:text-[13px] font-normal opacity-30 mx-1">|</span>
                                                        </>
                                                    )}
                                                    <span className={cn(
                                                        "text-[10px] md:text-[12px] font-bold truncate transition-colors duration-500", 
                                                        isActive ? "text-white" : (l.is_my_point ? "text-blue-300" : "text-rose-300")
                                                    )}>{l.point_type}</span>
                                                </div>
                                            )}
                                        </div>
                                        ...
                                        <div className={cn(
                                            "w-10 md:w-12 text-center font-black text-[10px] md:text-[12px] tabular-nums transition-colors duration-500", 
                                            isActive ? "text-cyan-300" : "text-slate-500"
                                        )}>
                                            {formatTime(l.video_timestamp)}
                                        </div>

                                        <div className="hidden md:flex w-16 justify-end gap-2 pr-1 shrink-0">
                                            <button 
                                                onClick={(e) => { e.stopPropagation(); setEditingLogId(l.id); }} 
                                                className={cn("hover:scale-125 transition-all outline-none", isActive ? "text-white/60 hover:text-white" : "text-slate-400 hover:text-white")}
                                            >
                                                <Edit2 className="w-4 h-4"/>
                                            </button>
                                            <button 
                                                onClick={(e) => { e.stopPropagation(); deleteLog(l.id); }} 
                                                className={cn("hover:scale-125 transition-all outline-none", isActive ? "text-rose-400 hover:text-rose-300" : "text-slate-500 hover:text-rose-500")}
                                            >
            ...
            <style jsx global>{`
                ...
                @media (max-width: 768px) {
                    body { overflow-x: hidden; height: auto; position: relative; }
