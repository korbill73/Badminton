...
export default function ProArchiveMainPage() {
    ...
    }, [player, activeLoop, isSequential, sequentialIndex, notes, activeFilter, selectedLoopIds]);
                            ...
                            <div key={m.id} onClick={() => { setStudyMatch(m); fetchNotes(m.id); }} className="group relative bg-[#111827] rounded-2xl md:rounded-[3.2rem] p-4 md:px-10 md:py-7 transition-all duration-500 flex flex-col md:flex-row items-center border border-white/10 hover:border-blue-500 shadow-inner hover:scale-[1.012] cursor-pointer gap-4 md:gap-0">
                                ...
                                <div className="md:hidden w-full flex flex-col gap-3">
                                    ...
                                    <div className="flex flex-col gap-1">
                                        <h3 className="text-sm font-black text-white truncate">{m.tournament} <span className="text-[10px] text-yellow-500 font-bold ml-1">@{m.location}</span></h3>
                                        <div className="flex items-center gap-2 text-[13px] font-black">
                                            <span className="text-sky-400 truncate">{m.player_name}{m.partner_name && `/${m.partner_name}`}</span>
                                            <span className="text-white/20 text-[10px]">VS</span>
                                            <span className="text-yellow-400 truncate">{m.opponent}{m.opponent_2_name && `/${m.opponent_2_name}`}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
...
function ProArchiveMobileView({ 
    ...
    const activeNote = isSequential 
        ? currentFilteredNotes[sequentialIndex] 
        : notes.find((n: any) => n.timestamp.startsWith(formatTime(activeLoop?.start || 0)));

    return (
        <div className="fixed inset-0 z-[2000] bg-black flex flex-col landscape:flex-row text-white overflow-hidden">
            {/* Portrait Header (Only visible in portrait) */}
            <div className={cn(
                "portrait:absolute portrait:top-0 portrait:left-0 portrait:right-0 portrait:z-50 portrait:h-14 portrait:bg-gradient-to-b portrait:from-black/90 portrait:to-transparent portrait:px-4 portrait:flex portrait:items-center portrait:justify-between transition-all duration-500 md:hidden", 
                !showControls && "portrait:-translate-y-full portrait:opacity-0",
                "landscape:hidden"
            )}>
                <div className="flex items-center gap-3">
                    <button onClick={onClose} className="p-2 bg-rose-600 text-white rounded-xl shadow-lg"><X className="w-4 h-4" /></button>
                    <div className="flex flex-col">
                        <h2 className="text-[11px] font-black truncate max-w-[120px]">
                            {activeFilter === '전체' ? studyMatch.player_name : `${activeFilter} 집중 분석`}
                        </h2>
                        <span className="text-[8px] font-black text-blue-400 uppercase tracking-widest">Mobile Cinema Mode</span>
                    </div>
                </div>
                <div className="flex items-center gap-2">
                    <button 
                        onClick={(e) => { e.stopPropagation(); toggleFullScreen(); }}
                        className="p-2 bg-white/10 hover:bg-white/20 rounded-xl border border-white/10"
                    >
            ...
            {/* Video Area (Main Content) - CLEAN IN LANDSCAPE */}
            <div className="flex-1 relative bg-black flex items-center justify-center overflow-hidden" onClick={() => setShowControls(!showControls)}>
                <div className="w-full h-full flex items-center justify-center">
                    <div id="pro-video-player" className="w-full landscape:h-full aspect-video landscape:aspect-auto" />
                </div>
                
                {/* Loop Info Overlay - ONLY IN PORTRAIT */}
                {(activeLoop || isSequential) && (
                    <div className={cn(
                        "portrait:absolute portrait:z-40 transition-all duration-500",
                        "portrait:bottom-28 portrait:left-4 portrait:right-4",
                        "landscape:hidden", // 가로모드에서는 사이드바에 표시
                        !showControls && "portrait:translate-y-16 portrait:opacity-0"
                    )}>
                        <div className="bg-black/60 backdrop-blur-md border border-white/10 p-3 rounded-2xl flex items-center justify-between shadow-2xl">
                            <div className="flex items-center gap-3 min-w-0">
                                <div className={cn("w-1.5 h-1.5 rounded-full animate-pulse shrink-0", isSequential ? "bg-emerald-500" : "bg-blue-500")} />
                                <div className="flex flex-col min-w-0">
                                    <p className="text-[8px] font-black text-white/50 uppercase tracking-widest truncate">
                                        {isSequential ? `${activeFilter} (${sequentialIndex + 1}/${currentFilteredNotes.length})` : '반복 중'}
                                    </p>
                                    <p className="text-xs font-black text-white truncate">{activeNote?.content || '전술 루프'}</p>
                                </div>
                            </div>
                            <button 
                                onClick={(e) => { e.stopPropagation(); setIsSequential(false); startNoteLoop(null); }} 
                                className="p-2 bg-rose-600/20 text-rose-500 border border-rose-500/30 rounded-xl active:scale-90 ml-2"
                            >
            ...
            {/* Controls Overlay - Portrait: Bottom Bar, Landscape: Right Sidebar (160px) */}
            <div className={cn(
                "z-50 bg-gradient-to-t from-black to-black/80 backdrop-blur-xl border-white/10 transition-all duration-500 transform",
                // Portrait Style
                "portrait:absolute portrait:bottom-0 portrait:left-0 portrait:right-0 portrait:p-4 portrait:pt-6 portrait:rounded-t-[2.5rem] portrait:border-t",
                !showControls && "portrait:translate-y-full",
                // Landscape Style (Sidebar)
                "landscape:relative landscape:w-[160px] landscape:h-full landscape:border-l landscape:p-3 landscape:flex landscape:flex-col landscape:gap-4",
                !showControls && "landscape:translate-x-full"
            )}>
                {/* Landscape Specific Top Header */}
                <div className="hidden landscape:flex flex-col gap-3">
                    <div className="flex items-center justify-between gap-2">
                        <button onClick={onClose} className="flex-1 p-2 bg-rose-600 text-white rounded-xl shadow-lg flex justify-center"><X className="w-5 h-5" /></button>
                        <button onClick={toggleFullScreen} className="flex-1 p-2 bg-white/10 rounded-xl border border-white/10 flex justify-center"><Maximize2 className="w-5 h-5" /></button>
                    </div>
                    <div className="flex flex-col text-center">
                        <h2 className="text-[10px] font-black text-blue-400 uppercase tracking-tighter">Cinema Training</h2>
                        <h3 className="text-[11px] font-black truncate">{studyMatch.player_name}</h3>
                    </div>
                ...
                {/* Filters Section */}
                <div className="space-y-3 landscape:space-y-1.5 flex-1 flex flex-col justify-center">
                    <div className="flex items-center justify-between px-1 portrait:mb-1 landscape:hidden">
                        <h3 className="text-[9px] font-black text-slate-400 uppercase tracking-wider">{activeFilter}</h3>
                        <div className="landscape:hidden">
                            {!isSequential && currentFilteredNotes.length > 0 && (
                                <button onClick={(e) => { e.stopPropagation(); setIsSequential(true); }} className="px-2 py-1 bg-emerald-600 text-white rounded-lg font-black text-[8px] flex items-center gap-1">
                                    <PlayCircle className="w-3 h-3" /> 재생
                                </button>
                            )}
                        </div>
                    </div>
                    
                    <div className="flex portrait:flex-row landscape:flex-col gap-2 w-full">
                        {filters.map(f => {
                            ...
                            return (
                                <button 
                                    ...
                                    className={cn(
                                        "flex-1 py-3 landscape:py-2.5 px-3 rounded-xl border-2 transition-all flex portrait:flex-col landscape:flex-row items-center portrait:justify-center landscape:justify-between gap-1 active:scale-95",
                                        isActive 
                                            ? "bg-blue-600 border-blue-400 text-white shadow-lg shadow-blue-600/30" 
                                            : "bg-white/5 border-white/10 text-white/40"
                                    )}
                                >
                                    <span className="text-[10px] landscape:text-[12px] font-black">{f}</span>
                                    <span className={cn("text-[8px] landscape:text-[10px] font-black", isActive ? "text-blue-200" : "text-white/20")}>{count}</span>
                                </button>
                            );
                        })}
                    </div>
                ...
                {/* Landscape Specific Bottom Info */}
                {(activeLoop || isSequential) && (
                    <div className="hidden landscape:flex flex-col gap-2 mt-auto p-2 bg-white/5 rounded-2xl border border-white/10">
                        <div className="flex items-center gap-2">
                             <div className={cn("w-2 h-2 rounded-full animate-pulse", isSequential ? "bg-emerald-500" : "bg-blue-500")} />
                             <span className="text-[9px] font-black uppercase text-white/50">{isSequential ? 'Sequential' : 'Looping'}</span>
                        </div>
                        <p className="text-[10px] font-black text-white leading-tight truncate">{activeNote?.content || 'Tactical Note'}</p>
                        <button 
                            onClick={(e) => { e.stopPropagation(); setIsSequential(false); startNoteLoop(null); }} 
                            className="w-full py-2 bg-rose-600 text-white rounded-lg flex items-center justify-center gap-2 text-[10px] font-black active:scale-95 transition-all"
                        >
            ...
            {/* Floating Toggle Controls Button (Visible when controls are hidden) */}
            <button 
