                ...
                <div className="space-y-12 md:space-y-16">
                    {Object.entries(groupedMatches).length === 0 ? (
                        <div className="h-[300px] md:h-[400px] bg-white/5 rounded-[2rem] md:rounded-[4rem] border border-dashed border-white/10 flex flex-col items-center justify-center gap-4 md:gap-6 px-4 text-center">
                            ...
                            <p className="text-slate-500 font-black text-base md:text-xl">기록된 경기가 없습니다. 새로운 경기를 등록해 주세요.</p>
                        </div>
                    ) : (
                        ...
                        .map(([tName, matches]) => {
                            ...
                            return (
                                <div key={tName} className="space-y-6 md:space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 mb-20 md:mb-32">
                                                ...
                                                <div className="flex flex-col gap-2 md:gap-3">
                                                    <div className="flex flex-col md:flex-row md:items-center gap-3 md:gap-10">
                                                        <h3 
                                                            ...
                                                            onClick={() => { setEditingTournament(matches[0].tournament); setTModalOpen(true); }}
                                                        >
                                                            {tName}
                                                        </h3>
                                                        <div className="flex flex-wrap items-center gap-x-5 gap-y-2 text-sm md:text-2xl font-black tracking-tighter">
                                                            <span className="flex items-center gap-2 text-yellow-500 drop-shadow-[0_0_15px_rgba(234,179,8,0.4)]">
                                                                <MapPin className="w-5 h-5 md:w-8 md:h-8" /> {matches[0].tournament?.location || '미지정'}
                                                            </span>
                                                            <span className="hidden md:block w-px h-8 bg-white/10" />
                                                            <span className="flex items-center gap-2 text-white/70">
                                                                <Calendar className="w-5 h-5 md:w-7 md:h-7 text-emerald-400" /> {matches[0].tournament?.start_date || '-'} ~ {matches[0].tournament?.end_date || '-'}
                                                            </span>
                                                            {matches[0].tournament?.result && (
                                                                <>
                                                                    ...
                                                                    <span className="flex items-center gap-2 text-amber-500 bg-amber-500/5 px-4 py-1.5 rounded-full border border-amber-500/20">
                                                                        <Trophy className="w-5 h-5 md:w-7 md:h-7" /> {matches[0].tournament.result}
                                                                    </span>
                                                                </>
                                                            )}
                                                        </div>
                                                    ...
                                                    <div className="flex flex-col md:flex-row md:items-center gap-4 md:gap-10 flex-1 relative z-10">
                                                        <div className="flex items-center gap-4 md:min-w-[400px] flex-1">
                                                            ...
                                                            <div className="md:hidden flex-1 flex flex-col gap-2">
                                                                ...
                                                                <div className="text-[11px] font-bold text-slate-400 whitespace-normal break-words border-t border-white/5 pt-1">
                                                                    {m.match_name || '매치 기록'}
                                                                </div>
                                                            </div>
