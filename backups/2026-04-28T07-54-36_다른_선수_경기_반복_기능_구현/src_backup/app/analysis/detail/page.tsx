...
import { 
    ...
    RotateCcw, ListFilter, CheckSquare, Square,
    BarChart3, Video, RefreshCcw
} from 'lucide-react';
...
const parseScore = (score: any) => {
    ...
    return nums && nums.length >= 2 ? nums.map(Number) : [0, 0];
};
...
function CockpitAnalysisContent() {
    ...
    const saveHybridMeta = async (updates: any) => {
    ...
    const recordPoint = async (isMe: boolean, type: string) => {
        ...
        try {
            ...
            // 1. Insert the new log first
            const { error: pErr } = await supabase.from('bd_point_logs').insert({
            ...
            // 2. FETCH ALL LOGS for this set with UNIFIED SORTING (Match UI sorting)
            const { data: allSetLogs } = await supabase.from('bd_point_logs')
                ...
                .order('id', { ascending: true }); // Final fallback for absolute order

            if (allSetLogs && allSetLogs.length > 0) {
                let rollingMe = 0, rollingOpp = 0;
                // Calculate all updates synchronously first to avoid race conditions
                const updates = allSetLogs.map(l => {
                    if (l.is_my_point) rollingMe++; else rollingOpp++;
                    const newScore = `${rollingMe}-${rollingOpp}`;
                    return { id: l.id, current_score: newScore };
                });

                // Batch update (parallelized but with pre-calculated values)
    ...
    const cSetLogs = useMemo(() => {
        return logs.filter(l => Number(l.set_number) === Number(currentSet))
        ...
        <div className="flex flex-col h-screen bg-[#080d1a] text-white font-sans overflow-hidden">
            ...
            <div className="flex-1 flex overflow-hidden p-2 gap-2">
                ...
                <div className="flex flex-col w-[420px] bg-[#0b1221] rounded-[2rem] border border-white/10 overflow-hidden shadow-2xl shrink-0 h-[82%] relative group/timeline">
                    ...
                    <div className="flex-1 overflow-y-auto p-2 space-y-2 bg-[#080d1a] custom-scrollbar-hidden">
                        {cSetLogs.map((l, idx) => {
                            const loop = rallyLoops[l.id];
                                ...
                                <div key={l.id} className={cn(
                                    ...
                                    <div className="flex items-center justify-between">
                                        ...
                                        <div className="flex items-center gap-2 shrink-0 bg-black/50 p-2 rounded-2xl border border-white/10 shadow-lg">
                                            ...
                                            <button onClick={async () => { if(confirm('삭제하시겠습니까?')) { const {error} = await supabase.from('bd_point_logs').delete().eq('id', l.id); if(!error) fetchData(); } }} className="ml-1 p-2 text-white/20 hover:text-rose-500 transition-all opacity-0 group-hover/row:opacity-100 hover:rotate-12 active:scale-75"><Trash2 className="w-4 h-4" /></button>
                                        </div>
                                    </div>
                                </div>
                            );
            ...
            <style jsx global>{`.custom-scrollbar-hidden::-webkit-scrollbar { width: 0px; height: 0px; } .custom-scrollbar::-webkit-scrollbar { width: 3px; } .custom-scrollbar::-webkit-scrollbar-thumb { background: rgba(255,255,255,0.05); border-radius: 10px; } @keyframes spin-slow { from { transform: rotate(0deg); } to { transform: rotate(360deg); } } .animate-spin-slow { animation: spin-slow 3s linear infinite; }`}</style>
        </div>
    );
}
