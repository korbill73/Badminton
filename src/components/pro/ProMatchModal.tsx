...
export default function ProMatchModal({ isOpen, onClose, match, players, onSave, tournamentId, matches }: ProMatchModalProps) {
    ...
    // UNIQUE DATA FOR AUTOCOMPLETE
    const existingTournaments = Array.from(new Set(matches?.map((m: any) => m.tournament).filter(Boolean))) as string[];
    const existingLocations = Array.from(new Set(matches?.map((m: any) => m.location).filter(Boolean))) as string[];
    ...
    const handleSuggest = (text: string, field: string) => {
        ...
        if (text.length > 0) {
            if (field === 'tournament') {
                const filtered = existingTournaments.filter(t => t.toLowerCase().includes(text.toLowerCase()));
                setTournamentSuggestions(filtered);
            } else if (field === 'location') {
                const filtered = existingLocations.filter(l => l.toLowerCase().includes(text.toLowerCase()));
    ...
    const selectSuggest = (val: any, field: string) => {
        if (field === 'tournament' || field === 'location') {
    ...
    return (
        <div className="fixed inset-0 z-[800] flex items-center justify-center p-0 md:p-6 bg-black/95 backdrop-blur-xl animate-in fade-in duration-300">
            <div className="bg-[#0f172a] text-white w-full max-w-7xl h-full md:h-fit md:max-h-[98vh] md:rounded-[3.5rem] border-x md:border border-white/10 shadow-[0_0_150px_rgba(37,99,235,0.25)] flex flex-col overflow-hidden relative">
                
                {/* Header */}
                <div className="bg-gradient-to-r from-blue-700 via-indigo-800 to-blue-900 px-6 md:px-10 py-4 md:py-5 shrink-0 flex items-center justify-between">
                    <div className="flex items-center gap-3 md:gap-5">
                        <div className="p-2 md:p-3 bg-white/10 rounded-xl md:rounded-2xl shadow-lg"><Zap className="w-5 h-5 md:w-6 md:h-6 text-yellow-400" /></div>
                        <h2 className="text-xl md:text-3xl font-black tracking-tighter">선배 전장 데이터 마스터</h2>
                    </div>
                    <button onClick={onClose} className="p-2 md:p-3 hover:bg-rose-600/20 text-white/50 hover:text-white rounded-xl md:rounded-2xl transition-all"><X className="w-6 h-6 md:w-8 md:h-8" /></button>
                </div>

                <div className="p-6 md:p-10 space-y-6 overflow-y-auto custom-scrollbar flex-1">
                    {/* Row 1: Tournament & Category */}
                    <div className="grid grid-cols-1 md:grid-cols-12 gap-4 md:gap-5 bg-blue-600/5 p-4 md:p-6 rounded-3xl md:rounded-[2.5rem] border border-blue-500/10">
                        <div className="md:col-span-1 space-y-1">
                             <p className="text-[10px] font-black text-slate-500 tracking-widest md:text-center">분류</p>
                        ...
                        <div className="md:col-span-3 space-y-1 relative">
                             ...
                             {activeSuggestField === 'tournament' && tournamentSuggestions.length > 0 && (
                                 <div className="absolute top-full left-0 right-0 mt-1 bg-[#1e293b] border border-white/10 rounded-2xl shadow-2xl z-[100] max-h-[150px] overflow-y-auto">
                                     {tournamentSuggestions.map((t, idx) => (
                                         <div key={idx} onClick={() => selectSuggest(t, 'tournament')} className="px-4 py-3 hover:bg-blue-600 transition-all cursor-pointer font-bold text-sm border-b border-white/5 last:border-0">{t}</div>
                                     ))}
                                 </div>
                             )}
                        </div>
                ...
                {/* Footer */}
                <div className="px-6 md:px-10 py-4 md:py-6 bg-black/60 border-t border-white/10 flex items-center justify-between shrink-0">
                    <button onClick={onClose} className="text-slate-500 font-black hover:text-white transition-all text-[10px] md:text-[11px] tracking-widest underline underline-offset-4">
                        기록 취소
                    </button>
                    <button 
                        onClick={handleFinalSave} 
                        className="bg-blue-600 px-8 md:px-14 py-3 md:py-4 rounded-2xl md:rounded-[2rem] font-black text-white hover:bg-blue-500 transition-all shadow-2xl active:scale-95 flex items-center gap-3 md:gap-5 group text-lg md:text-2xl"
                    >
                        <span>데이터 분석 완료</span>
